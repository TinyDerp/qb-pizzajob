...
local PlayerJob = QBCore.Functions.GetPlayerData().job --new

RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    PlayerData = QBCore.Functions.GetPlayerData()
	PlayerJob = QBCore.Functions.GetPlayerData().job --new
end)

RegisterNetEvent("QBCore:Client:OnJobUpdate", function(JobInfo) --all new
    PlayerData = QBCore.Functions.GetPlayerData()
    PlayerJob = JobInfo
end)
...
function PullOutVehicle()
    if ownsVan == true then
        QBCore.Functions.Notify("You already have a car! Go and collect it or end your job.", "error")
    elseif ownsVan == false then
        coords = Config.Zones.Spawn.Pos
        QBCore.Functions.SpawnVehicle('foodcar4', function(veh)
...
CreateThread(function()
    while true do
        Wait(0)
        if Hired and PlayerJob.name == "pizzaboy" then -- new: and PlayerJob.name == "pizzaboy"
...
CreateThread(function()
    while true do
        ...
        if dist <= 20.0 and Hired and PlayerJob.name == "pizzaboy" and (not HasPizza) then --new: and PlayerJob.name == "pizzaboy"
            local DeliveryPoint = {
                ["x"] = xxx,
                ["y"] = yyy,
                ["z"] = zzz
            }
            DrawText3Ds(DeliveryPoint["x"],DeliveryPoint["y"],DeliveryPoint["z"], "Take ~y~Pizza~s~ from ~b~foodcar4~s~!")
            local Vehicle = GetClosestVehicle(pos, 6.0, 0, 70)
            if IsVehicleModel(Vehicle, GetHashKey('foodcar4')) then
        ...
        elseif dist <= 25 and HasPizza and Hired and PlayerJob.name == "pizzaboy" then --new: and PlayerJob.name == "pizzaboy"
            local DeliveryPoint = {
                ["x"] = xxx,
                ["y"] = yyy,
                ["z"] = zzz
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('foodcar4')) then
            ...
            QBCore.Functions.Notify("You must return to pizza foodcar4!", "error")
            QBCore.Functions.Notify("If you lost the foodcar4 cancel the job on foot", "error")
        end
    else
        InService = false
        BlipSell = nil
